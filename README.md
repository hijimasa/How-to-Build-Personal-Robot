...
- 本リポジトリの本文は AI（Claude / Codex）を用いて作成・編集しています。
- 内容の正確性・完全性・最新性は保証しません。実装時は一次情報で検証してください。
- 本文（`docs/` 配下のドキュメント本文・図版のうち特記なきもの）：CC BY 4.0
- コード（設定ファイル、スクリプト、CI 設定など）：MIT
- 画像・図版・外部素材に個別ライセンス表記がある場合は、その表記を優先します。

詳細:

- `docs/appendix/licenses-and-attributions.ja.md`
- `LICENSE`（コード向け）
- `LICENSE-CONTENT.md`（本文向け）
