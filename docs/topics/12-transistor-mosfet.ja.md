...
!!! info "初回はこの最短ルートだけで OK"
    迷ったら、最初の 1 回は次のルールだけ守って組んでください。

    1. **100 mA 未満の小負荷**（LED 多灯、小ブザーなど）：NPN + ベース抵抗 1 kΩ
    2. **100 mA 以上** または将来モータに発展しそう：ロジックレベル MOSFET + ゲート抵抗 220 Ω + プルダウン 10 kΩ
    3. **コイル負荷**（リレー、ソレノイド、モータ）ではフライホイールダイオードを必ず入れる

    まずはこのテンプレで 1 回動かし、詳細な定数最適化はその後に行うのが挫折しにくい進め方です。
