...
!!! info "最短で進める人向け（組立中チェックの芯）"
    1. ねじは「仮締め → 干渉確認 → 本締め」の順に固定する
    2. 各フェーズの区切りで、可動部を必ず手回し確認する（通電前）
    3. 配線は 10〜15 cm ごとに固定し、可動部から 10 mm 以上離す
    4. 接着や最終固定など戻れない操作の直前で、必ず 1 回停止して再確認する
